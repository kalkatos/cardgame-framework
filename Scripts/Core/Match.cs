...
namespace CardgameCore
{
	...
	public class Match : MonoBehaviour
	{
		...
		private int currentCommandIdentation;
		//Match control
		...
		private IEnumerator OnMessageSentTrigger (string message, string additionalInfo)
		{
			...
			if (DebugLog)
			{
				...
				foreach (var item in OnMessageSentRules)
				{
					...
					{
						CustomDebug.Log($"Evaluating rule: {item.Value.name}", 1);
						string conditionToString = item.Value.condition.ToString();
		...
		{
			instance.commands.Enqueue(new SingleZoneCommand(CommandType.UseZone, UseZonePrivate, zone, additionalInfo));
		}

		public static void OrganizeZone (Zone zone, string additionalInfo = "")
		{
			instance.commands.Enqueue(new SingleZoneCommand(CommandType.OrganizeZone, OrganizeZonePrivate, zone, additionalInfo));
		}
